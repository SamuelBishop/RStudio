...
purchase %>% 
  ...
  summarise(mean_days = mean(days_to_event)) %>%          # summarize days_to_event
...
purchase_latlong <- purchase %>% 
                    ...
                    summarize_all(.funs = list(mean=mean)) %>% # applies mean to all the variables selected except the grouping variable
